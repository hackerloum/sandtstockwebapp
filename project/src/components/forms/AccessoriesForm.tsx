...
export const AccessoriesForm: React.FC<AccessoriesFormProps> = ({
  product,
  onSave,
  onClose,
  isOpen
  ...
  const [formData, setFormData] = useState({
    ...
    item_number_suffix: '',
    commercial_name: '',
    product_type: 'Accessories' as ProductType,
    brand_id: '',
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        category: product.category || 'Perfume Accessories',
        concentration: product.concentration || '',
        size: product.size || 0,
        gross_weight: product.gross_weight || 0.2,
        tare_weight: product.tare_weight || 0.05,
        net_weight: product.net_weight || 0.15,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Saving accessories product:', finalData);
    onSave(finalData);
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={product ? `Edit ${product.product_type}` : 'Add New Accessory'}
      size="xl"
      footer={
        <>
          ...
          <Button variant="primary" onClick={handleSubmit}>
            {product ? 'Update Accessory' : 'Add Accessory'}
          </Button>
        </>
      }
    >
      <form className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField label="Initial Stock Quantity" required>
              ...
              />
              <p className="text-sm text-gray-600 mt-1">This will be the starting stock level for this accessory</p>
            </FormField>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <FormField label="Accessory Name" required>
              <Input
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <FormField label="Accessory Type" required>
              <Select
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField label="Weight (kg)" required>
              <Input
                ...
                className="text-lg font-medium"
              />
              <p className="text-sm text-gray-600 mt-1">Total weight of the accessory</p>
            </FormField>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="Description">
            <TextArea
              value={formData.fragrance_notes}
              onChange={(e) => setFormData(prev => ({ ...prev, fragrance_notes: e.target.value }))}
              placeholder="Describe the accessory features and specifications..."
              rows={3}
            />
          </FormField>
              ...
              <div className="flex items-center space-x-4">
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <span className="text-sm">Sample Accessory</span>
                </label>
              </div>
              <p className="text-sm text-gray-600 mt-1">Mark if this is a sample accessory</p>
            </FormField>
          </div>
        </div>
