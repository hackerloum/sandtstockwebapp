...
export const CrimpForm: React.FC<CrimpFormProps> = ({
  product,
  onSave,
  onClose,
  isOpen
  ...
  const [formData, setFormData] = useState({
    ...
    item_number_suffix: '',
    commercial_name: '',
    product_type: 'Crimp' as ProductType,
    brand_id: '',
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        category: product.category || 'Crimping Tools',
        concentration: product.concentration || '',
        size: product.size || 0,
        gross_weight: product.gross_weight || 0.5,
        tare_weight: product.tare_weight || 0.1,
        net_weight: product.net_weight || 0.4,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Saving crimp product:', finalData);
    onSave(finalData);
  };

  const crimpTypeOptions = [
    { value: 'Manual Crimper', label: 'Manual Crimper' },
    { value: 'Electric Crimper', label: 'Electric Crimper' },
    { value: 'Crimping Pliers', label: 'Crimping Pliers' },
    { value: 'Crimp Dies', label: 'Crimp Dies' },
    { value: 'Crimp Accessories', label: 'Crimp Accessories' }
  ];

  const crimpSizeOptions = [
    { value: 'Small', label: 'Small (5-10mm)' },
    { value: 'Medium', label: 'Medium (10-15mm)' },
    { value: 'Large', label: 'Large (15-20mm)' },
    { value: 'Extra Large', label: 'Extra Large (20mm+)' }
  ];
  ...
  const itemNumberPrefixOptions = [
    ...
    { value: 'CR', label: 'CR - Crimp' },
    { value: 'TK', label: 'TK - Tool Kit' }
  ];
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={product ? `Edit ${product.product_type}` : 'Add New Crimp Tool'}
      size="xl"
      footer={
        <>
          ...
          <Button variant="primary" onClick={handleSubmit}>
            {product ? 'Update Crimp Tool' : 'Add Crimp Tool'}
          </Button>
        </>
      }
    >
      <form className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField label="Initial Stock Quantity" required>
              ...
              />
              <p className="text-sm text-gray-600 mt-1">This will be the starting stock level for this crimp tool</p>
            </FormField>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <FormField label="Tool Name" required>
              <Input
                ...
                placeholder="e.g., Professional Crimper"
              />
            </FormField>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <FormField label="Crimp Type" required>
              <Select
                value={formData.category}
                onChange={(e) => setFormData(prev => ({ ...prev, category: e.target.value }))}
                options={crimpTypeOptions}
              />
            </FormField>

            <FormField label="Crimp Size">
              <Select
                value={formData.concentration}
                onChange={(e) => setFormData(prev => ({ ...prev, concentration: e.target.value }))}
                options={crimpSizeOptions}
              />
            </FormField>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField label="Weight (kg)" required>
              <Input
                ...
                className="text-lg font-medium"
              />
              <p className="text-sm text-gray-600 mt-1">Total weight of the tool</p>
            </FormField>
            ...
            <FormField label="Dimensions (cm)">
              <Input
                ...
                placeholder="e.g., 15x8x3"
                className="text-lg font-medium"
              />
              <p className="text-sm text-gray-600 mt-1">Length x Width x Height</p>
            </FormField>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="Tool Description">
            <TextArea
              value={formData.fragrance_notes}
              onChange={(e) => setFormData(prev => ({ ...prev, fragrance_notes: e.target.value }))}
              placeholder="Describe the crimp tool features and specifications..."
              rows={3}
            />
          </FormField>
              ...
              <div className="flex items-center space-x-4">
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <span className="text-sm">Demo/Sample Tool</span>
                </label>
              </div>
              <p className="text-sm text-gray-600 mt-1">Mark if this is a demo or sample tool</p>
            </FormField>
          </div>
        </div>
